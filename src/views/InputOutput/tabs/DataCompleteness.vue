...
const yearOptions = Array.from({ length: 6 }).map((_, i) => ({ label: String(new Date().getFullYear() - i), value: String(new Date().getFullYear() - i) }));
...
const makeColumns = () => {
  ...
  for (let i = 1; i <= 12; i++) {
    cols.push({ title: `${i}月`, dataIndex: `m${i}`, key: `m${i}`, width: 80, customRender: ({ text }: any) => `${text}%` });
  }
  columns.value = cols;
};
