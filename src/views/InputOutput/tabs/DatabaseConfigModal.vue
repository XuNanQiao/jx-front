<template>
  <a-modal v-model:open="visibleLocal" :confirm-loading="loadingLocal" :title="isViewMode ? '查看数据库连接配置' : form.id ? '编辑数据库连接配置' : '新建数据库连接配置'" :width="700" :ok-text="isViewMode ? '关闭' : '保存'" :cancel-button-props="{ style: { display: isViewMode ? 'none' : 'inline-block' } }" cancel-text="取消" @ok="onOk" @cancel="onCancel">
    <a-form ref="formRef" :model="form" :rules="rules" layout="vertical" :disabled="isViewMode">
      <a-row :gutter="16">
        <a-col :span="24">
          <a-form-item label="数据库类别" name="database_category">
      ...
      <template v-if="form.database_category">
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="ip地址" name="database_connection_config.ip_address">
              <a-input v-model:value="form.database_connection_config.ip_address" placeholder="例如: 192.168.1.11" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="端口号" name="database_connection_config.port">
      ...
      <template v-if="form.database_category == 'Postgresql' || form.database_category == 'MySQL'">
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="密码" name="database_connection_config.password">
              <a-input-password v-model:value="form.database_connection_config.password" placeholder="请输入密码" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="账号" name="database_connection_config.username">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="数据库名" name="database_connection_config.database_name">
              <a-input v-model:value="form.database_connection_config.database_name" placeholder="请输入数据库名" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="表名" name="database_connection_config.table_name">
      ...
      <template v-else-if="form.database_category === 'InfluxDB'">
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="组织" name="database_connection_config.organization">
              <a-input v-model:value="form.database_connection_config.organization" placeholder="请输入组织名" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="令牌(Token)" name="database_connection_config.token">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="桶名" name="database_connection_config.bucket">
